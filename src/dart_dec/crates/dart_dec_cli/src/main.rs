...
fn cmd_decompile(binary_path: &PathBuf, cli: &Cli) -> Result<()> {
    ...
    // Map Flutter versions to Dart SDK versions
    let (dart_major, dart_minor, dart_patch) = map_flutter_to_dart(
        version.major, version.minor, version.patch
    );

    let profile = resolver
        .resolve(dart_major, dart_minor, dart_patch)
        .ok_or_else(|| anyhow::anyhow!("No profile found for Dart {}.{}.{} (detected: {})", dart_major, dart_minor, dart_patch, version))?
        .clone();
...
/// Map Flutter SDK version to Dart SDK version.
/// Flutter bundles a specific Dart SDK, and the version string in the binary
/// is often the Flutter version, not the Dart one.
fn map_flutter_to_dart(major: u32, minor: u32, patch: u32) -> (u32, u32, u32) {
    // If version looks like a normal Dart version (2.x or 3.0-3.9), use as-is
    if major == 2 || (major == 3 && minor <= 9) {
        return (major, minor, patch);
    }

    // Flutter version -> Dart SDK mapping (major.minor only)
    // Flutter 3.35.x = Dart 3.9.x
    // Flutter 3.32.x = Dart 3.8.x
    // Flutter 3.29.x = Dart 3.7.x
    ...
    if major == 3 {
        ...
        tracing::info!(
            "Mapped Flutter {}.{}.{} -> Dart 3.{}.{}",
            major, minor, patch, dart_minor, patch
        );
        return (3, dart_minor, patch);
    }
